...
class Ramificacion_y_salto(Queue):
...
class Ramificacion_y_salto_con_subestimacion(Queue):
    ...
    def extend(self, items):
        self.A.extend(items)
        self.A.sort(key=(lambda item: item.path_cost + self.problem.h(item)), reverse=True)
